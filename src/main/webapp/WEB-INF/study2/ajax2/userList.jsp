...
<head>
	...
	<script>
		...
		function userUpdate(){
			...
			if(mid.trim() == ""){
				alert("수정할 아이디를 알려주세요. 수정하기 싫으시면 그대로 둬 주세요.");
				$("#mid").focus();
				return false;
			}
			else if(name.trim() == ""){
				alert("수정할 이름을 알려주세요. 수정하기 싫으시면 그대로 둬 주세요.");
				$("#name").focus();
				return false;
			}
			else if(age.trim() == ""){
				alert("수정할 나이를 알려주세요. 수정하기 싫으시면 그대로 둬 주세요.");
				$("#age").focus();
				return false;
			}
			...
			$.ajax({
				...
				data : {idx:idx ,mid:mid,name:name,age:age,address:address},
				success:function(res){
					alert(res);
				},
				error : function(){
	...
	<form>
		<table class="table table-bordered">
			...
			<tr>
				...
				<input type="button" value="전체보기" onclick="location.herf='${ctp}/UserList.st'" class="btn btn-secondary"/></td>
			</tr>
		</table>
	</form>
		<h2>user 전체 리스트</h2>
			...
			<c:forEach var="vo" items="${vos}" varStatus="st">
				<tr>
					...
					<td>
						...
						<a href="javascript:userSearch(${vo.idx})" class="btn btn-danger">개별조회</a>
					</td>
				</tr>
			</c:forEach>
