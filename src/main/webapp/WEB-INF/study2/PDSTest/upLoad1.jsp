...
<head>
	...
	<script>
		...
		function fCheck(){
			...
			if(ext1 != "JPG" && ext1 != "GIF" && ext1 != "ZIP" && ext1 != "HWP" && ext1 != "PNG" && ext1 != "PPT"){
...
<p><br/></p>	
	<div class="container">
		<h2>파일 업로드 처리 (싱글파일 업로드)</h2>
		<p>cos 라이브러리를 이용한 파일 업로드</p>
		<div>(https://servlets.com/cos/)</div>
		<hr/>
		<form name="myform" method="post" action="${ctp}/FileUpLoad1Ok.st" enctype="multipart/form-data"> <!-- 파일 전송시에는 무조건 인코딩 타입(enctype)을 써주어야함 -->
			파일명 :
