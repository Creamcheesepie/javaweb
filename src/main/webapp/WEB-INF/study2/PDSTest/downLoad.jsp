	...
	<div class="container">
		...
		<c:forEach var="file" items="${files}" varStatus="st">
			<tr>	
				<td>${st.count}</td>
				<td>
					<a href="${ctp}/images/PDSTest/${file}" download="${file}">
						${file}
					</a>
				</td>
				<td>
					...
					<c:if test="${extName == 'png' ||extName == 'gif' || extName == 'jpg' }">그림파일</c:if>
				</td>
				<td>
<%-- 					<input type="button" value="자바다운로드" onclick="javaDownLoad('${file}')" class="btn btn-warning"/> --%>	
					<input type="button" value="자바다운로드" onclick="location.href='${ctp}/FileDownLoad.st?file=${file}'" class="btn btn-warning"/>/	
					<input type="button" value="자바삭제" onclick="fileDelete('${file}')" class="btn btn-danger"/>	
				</td>
			</tr>
		</c:forEach>
