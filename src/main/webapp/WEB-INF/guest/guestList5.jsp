	...
	<div class="container">
		...
		<div class="text-center">
			...
			<c:forEach var="i" begin="${curBlock*blockSize+1}" end="${curBlock*blockSize+blockSize}" varStatus="st">
				<c:if test="${i== pag}"><font color="red">${i}</font></c:if>
				<c:if test="${i!= pag}"><a href="${ctp}/GuestList.gu?pag=${i}">${i}</a></c:if>
			</c:forEach>
			<c:if test="${curBlock<lastBlock}"><a href="${ctp}/GuestList.gu?pag=${(curBlock+1)*blockSize+1}">다음블록</a></c:if>
			<c:if test="${pag<totalPage}"><a href="${ctp}/GuestList.gu?pag=${totalPage}">마지막페이지</a></c:if>
		</div>
	</div>
<p><br/></p>
